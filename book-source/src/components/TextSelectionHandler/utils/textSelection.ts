 ...
 * Modified to include code blocks and filter main content only
 */
...
export function getCurrentSelection(): TextSelection | null {
  ...
  // Ignore selections in input fields or textareas (but INCLUDE code blocks)
  ...
  // Only exclude input/textarea (NOT code blocks - user wants to ask about code)
  if (element.closest('input, textarea')) {
    return null;
  }

  // Only allow selections in main content area
  const mainContent = document.querySelector('main, article, .markdown, [role="main"]');

  if (mainContent && !mainContent.contains(element)) {
    return null; // Selection is outside main content
  }

  // Exclude navigation, headers, footers, sidebars
  if (element.closest('nav, header, footer, aside, .navbar, .sidebar, .menu')) {
    return null;
  }
